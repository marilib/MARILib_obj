...
def analyse_data_base(oag_file_name, range_interval, capacity_interval, airport="all", origin=True, destination=True):
    ...
    if airport=="all":
        ...
        if origin and not destination:
            oag_df = oag_df_raw[oag_df_raw['Origin']==airport].copy()
        elif not origin and destination:
            oag_df = oag_df_raw[oag_df_raw['Destination']==airport].copy()
        elif origin and destination:
            oag_df = oag_df_raw[(oag_df_raw['Origin']==airport)|(oag_df_raw['Destination']==airport)].copy()
...
if __name__ == '__main__':
    ...
    # airport = "TLS"     # Toulouse IATA codes : TLS
    airport = "CCF"     # Carcassonne IATA codes : CCF

    data_matrix, oag_data = analyse_data_base(path_to_oag_file, range_interval, capacity_interval, airport, origin=True, destination=True)
