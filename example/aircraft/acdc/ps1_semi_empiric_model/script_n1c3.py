...
A = np.vstack([param*param, param, np.ones(n)]).T                # Need to transpose the stacked matrix

B = dict["Price"] - dict["nEngine"]*(0.045*dict["SLST"]+5.5)
...
plt.plot(B, B, 'green')
plt.grid(True)
plt.suptitle('Script_n3 : residual = '+"%.0f"%res, fontsize=14)
